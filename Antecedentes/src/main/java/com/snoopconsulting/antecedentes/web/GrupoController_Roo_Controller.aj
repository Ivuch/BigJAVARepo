...
import com.snoopconsulting.antecedentes.web.GrupoController;
...
privileged aspect GrupoController_Roo_Controller {
    ...
    @Autowired
    GrupoRepository GrupoController.grupoRepository;
    ...
    public String GrupoController.show(@PathVariable("id") Long id, Model uiModel) {
        uiModel.addAttribute("grupo", grupoRepository.findOne(id));
    ...
    public String GrupoController.list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, Model uiModel) {
        if (page != null || size != null) {
            ...
            uiModel.addAttribute("grupoes", grupoRepository.findAll(new org.springframework.data.domain.PageRequest(firstResult / sizeNo, sizeNo)).getContent());
            float nrOfPages = (float) grupoRepository.count() / sizeNo;
            uiModel.addAttribute("maxPages", (int) ((nrOfPages > (int) nrOfPages || nrOfPages == 0.0) ? nrOfPages + 1 : nrOfPages));
        } else {
            uiModel.addAttribute("grupoes", grupoRepository.findAll());
        }
        return "grupoes/list";
    }
    ...
    public String GrupoController.updateForm(@PathVariable("id") Long id, Model uiModel) {
        populateEditForm(uiModel, grupoRepository.findOne(id));
        return "grupoes/update";
    }
